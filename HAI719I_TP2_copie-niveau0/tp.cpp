...
struct TriangleVArray {
    ...
    void initBuffers(){
        ...
        // on initialise les donnees utilisees pour le buffer_coord_txt (buffer pour les textures)
        ...
        // on cree un buffer pour les textures de la meme maniere
        glGenBuffers(1, &buffer_coord_txt);
        glBindBuffer(GL_ARRAY_BUFFER, buffer_coord_txt);
        glBufferData(GL_ARRAY_BUFFER, sizeof(g_coord_buffer_data), g_coord_buffer_data, GL_STATIC_DRAW);
        ...
        unsigned char *data = stbi_load("brique.png", &width , &height , &nrChannels , 0);
        if (data) {
    ...
    void draw (){
        ...
        // glBind ca signifie: a partir de mtn je m'occupe de ce buffer ci 
        // on peut en mettre regulierement qd on deal avec plusieurs buffers
        glBindBuffer(GL_ARRAY_BUFFER, vertexbuffer);
        // aucune raison de normaliser ici donc GL_FALSE
        ...
        //Pensez à desactiver les AttributArray

        // activer les deux buffers (index 0 puis 1 puis 2)
        glEnableVertexAttribArray(0);
        glEnableVertexAttribArray(1);
        glEnableVertexAttribArray(2); // ajoute pour la texture

        // Draw the triangle !
        glDrawArrays(GL_TRIANGLES, 0, 3);

        // désactiver les deux buffers (index 0 puis 1 puis 2)
        glDisableVertexAttribArray(0);
        glDisableVertexAttribArray(1);
        glDisableVertexAttribArray(2); // ajoute pour la texture
...
struct Mesh {
    ...
    void draw (){
        ...
        // activer les deux buffers (index 0 puis 1)
        glEnableVertexAttribArray(0);
        glEnableVertexAttribArray(1);
...
void draw () {
    ...
    GLuint loc = glGetUniformLocation(programID, "scale");
    ...
    GLuint loc1 = glGetUniformLocation(programID, "translation");

    // Ajouter une translation en envoyant un vec3
    glUniform3f(loc1, translate[0], translate[1], translate[2]);
